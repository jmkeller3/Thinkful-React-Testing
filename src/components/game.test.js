...
describe("<Game />", () => {
  ...
  it("Can make guesses", () => {
    ...
    expect(wrapper.state("guesses")).toEqual([25, 60, 81]);
    expect(wrapper.state("feedback")).toEqual("You're Warm.");
    wrapper.instance().makeGuess(96);
    expect(wrapper.state("guesses")).toEqual([25, 60, 81, 96]);
    expect(wrapper.state("feedback")).toEqual("You're Hot!");
    wrapper.instance().makeGuess(100);
    expect(wrapper.state("guesses")).toEqual([25, 60, 81, 96, 100]);
  ...
  it("Can generate aural updates", () => {
    ...
    expect(wrapper.state("auralStatus")).toEqual(
      "Here's the status of the game right now: You're Warm. You've made 3 guesses. In order of most- to least-recent, they are: 90, 2, 25"
    );
  });
});
